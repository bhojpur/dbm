...
// Context represents a context for ORM tag parse.
type Context struct {
	tag
	tagUname        string
...
// Handler describes tag handler for ORM
...
func IndexTagHandler(ctx *Context) error {
	if len(ctx.params) > 0 {
		ctx.indexNames[ctx.params[0]] = schema.IndexType
	} else {
		ctx.isIndex = true
	}
	return nil
...
func UniqueTagHandler(ctx *Context) error {
	if len(ctx.params) > 0 {
		ctx.indexNames[ctx.params[0]] = schema.UniqueType
	} else {
		ctx.isUnique = true
	}
	return nil
...
func SQLTypeTagHandler(ctx *Context) error {
	ctx.col.SQLType = schema.SQLType{Name: ctx.tagUname}
