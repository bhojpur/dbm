...
func (session *Session) ContextCache(context ctxsvr.ContextCache) *Session {
...
func getField(dataStruct *reflect.Value, table *schemasvr.Table, colName string, idx int) (*schemasvr.Column, *reflect.Value, error) {
...
func (session *Session) rows2Beans(rows *core.Rows, fields []string, types []*sql.ColumnType,
	table *schemasvr.Table, newElemFunc func([]string) reflect.Value,
	sliceValueSetFunc func(*reflect.Value, schemasvr.PK) error) error {
	for rows.Next() {
...
func (session *Session) convertBeanField(col *schemasvr.Column, fieldValue *reflect.Value,
	scanResult interface{}, table *schemasvr.Table) error {
	...
	case reflect.Struct:
		if fieldType.ConvertibleTo(schemasvr.BigFloatType) {
		...
		if fieldType.ConvertibleTo(schemasvr.TimeType) {
			...
			var pk = make(schemasvr.PK, len(table.PrimaryKeys))
			...
			if !pk.IsZero() {
				...
				// however, also need to consider adding a 'lazy' attribute to ORM tag which allow hasOne
...
func (session *Session) slice2Bean(scanResults []interface{}, fields []string, bean interface{}, dataStruct *reflect.Value, table *schemasvr.Table) (schemasvr.PK, error) {
	defer func() {
