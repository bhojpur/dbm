...
//revive:disable
func (session *Session) cacheUpdate(table *schemasvr.Table, tableName, sqlStr string, args ...interface{}) error {
	if table == nil ||
		session.tx != nil {
	...
	ids, err := cache.GetCacheSql(cacher, tableName, newsql, args[nStart:])
	if err != nil {
	    ...
	    session.engine.LogDebug("[orm:cacheUpdate] del cached sql:", tableName, newsql, args)
	...
	for _, id := range ids {
		...
		if bean := cacher.GetBean(tableName, sid); bean != nil {
			...
			for idx, kv := range kvs {
				...
				colName = schemasvr.CommonQuoter.Trim(colName)
