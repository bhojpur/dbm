...
func (db *mssql) Version(ctx context.Context, queryer core.Queryer) (*schemasvr.Version, error) {
...
func (db *mssql) SQLType(c *schemasvr.Column) string {
	var res string
	switch t := c.SQLType.Name; t {
	case schemasvr.Bool, schemasvr.Boolean:
		res = schemasvr.Bit
	...
	case schemasvr.Bytea, schemasvr.Binary:
		res = schemasvr.VarBinary
		if c.Length == 0 {
			c.Length = 50
		}
	...
	case schemasvr.TimeStamp, schemasvr.DateTime:
		if c.Length > 3 {
			res = "DATETIME2"
		} else {
	...
	case schemasvr.TimeStampz:
		res = "DATETIMEOFFSET"
		c.Length = 7
	case schemasvr.MediumInt, schemasvr.TinyInt, schemasvr.SmallInt, schemasvr.UnsignedMediumInt, schemasvr.UnsignedTinyInt, schemasvr.UnsignedSmallInt:
		res = schemasvr.Int
	case schemasvr.Text, schemasvr.MediumText, schemasvr.TinyText, schemasvr.LongText, schemasvr.Json:
	...
	if res == schemasvr.Int || res == schemasvr.Bit {
		return res
	}
	hasLen1 := (c.Length > 0)
	hasLen2 := (c.Length2 > 0)
...
func (db *mssql) ModifyColumnSQL(tableName string, col *schemasvr.Column) string {
...
func (db *mssql) GetColumns(queryer core.Queryer, ctx context.Context, tableName string) ([]string, map[string]*schemasvr.Column, error) {
	args := []interface{}{}
	...
	for rows.Next() {
		...
		case "NVARCHAR":
			col.SQLType = schemasvr.SQLType{Name: schemasvr.NVarchar, DefaultLength: 0, DefaultLength2: 0}
			if col.Length > 0 {
				col.Length /= 2
				col.Length2 /= 2
			}
		case "DATETIME2":
			col.SQLType = schemasvr.SQLType{Name: schemasvr.DateTime, DefaultLength: 7, DefaultLength2: 0}
			col.Length = scale
		case "DATETIME":
			col.SQLType = schemasvr.SQLType{Name: schemasvr.DateTime, DefaultLength: 3, DefaultLength2: 0}
			col.Length = scale
		case "IMAGE":
			col.SQLType = schemasvr.SQLType{Name: schemasvr.VarBinary, DefaultLength: 0, DefaultLength2: 0}
		case "NCHAR":
			if col.Length > 0 {
				col.Length /= 2
				col.Length2 /= 2
			...
			fallthrough
		default:
			if _, ok := schemasvr.SqlTypes[ct]; ok {
				col.SQLType = schemasvr.SQLType{Name: ct, DefaultLength: 0, DefaultLength2: 0}
			} else {
...
func (db *mssql) GetTables(queryer core.Queryer, ctx context.Context) ([]*schemasvr.Table, error) {
	args := []interface{}{}
...
func (db *mssql) GetIndexes(queryer core.Queryer, ctx context.Context, tableName string) (map[string]*schemasvr.Index, error) {
...
func (db *mssql) CreateTableSQL(ctx context.Context, queryer core.Queryer, table *schemasvr.Table, tableName string) (string, bool, error) {
	if tableName == "" {
...
func (p *odbcDriver) Parse(driverName, dataSourceName string) (*URI, error) {
	...
	return &URI{DBName: dbName, DBType: schemasvr.MSSQL}, nil
}
